...
/**
 * This file contains the definition for the library class for mowaac submission plugin
 *
...
// File area for mowaac submission assignment.
define('ASSIGNSUBMISSION_mowaac_FILEAREA_SUBMISSION', 'submissions_mowaac');
// File area for mowaac tests to be uploaded by the teacher.
define('ASSIGNSUBMISSION_mowaac_FILEAREA_TEST', 'tests_mowaac');

/**
 * library class for mowaac submission plugin extending submission plugin base class
 *
 * @package assignsubmission_mowaac
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assign_submission_mowaac extends assign_submission_plugin {

    // Database table names.
    const TABLE_ASSIGNSUBMISSION_mowaac = "assignsubmission_mowaac";
    const TABLE_mowaac_TESTRESULT = "mowaac_testresult";
    const TABLE_mowaac_TESTFAILURE = "mowaac_testfailure";
    const TABLE_mowaac_COMPILATIONERROR = "mowaac_compilationerror";

    const COMPONENT_NAME = "assignsubmission_mowaac";

    /**
     * Get the name of the mowaac submission plugin
     * @return string
     */
    public function get_name() {
        return get_string("mowaac", self::COMPONENT_NAME);
    }

    /**
     * Get mowaac submission information from the database
     *
     * @param int $submissionid
     * @return mixed
     */
    private function get_mowaac_submission($submissionid) {
        global $DB;
        return $DB->get_record(self::TABLE_ASSIGNSUBMISSION_mowaac, array('submission_id' => $submissionid));
    }

    /**
     * Get the default setting for mowaac submission plugin
     *
    ...
    public function get_settings(MoodleQuickForm $mform) {
        ...
        $mform->addElement("filemanager", "mowaactests", $name, null, $fileoptions);
        $mform->addHelpButton("mowaactests",
            "setting_unittests",
            "assignsubmission_mowaac");
        $mform->disabledIf('mowaactests', 'assignsubmission_mowaac_enabled', 'notchecked');
    }

    /**
     * Save the settings for mowaac submission plugin
     *
     * @param stdClass $data
     * @return bool
     */
    public function save_settings(stdClass $data) {
        if (isset($data->mowaactests)) {
            file_save_draft_area_files($data->mowaactests, $this->assignment->get_context()->id,
                self::COMPONENT_NAME, ASSIGNSUBMISSION_mowaac_FILEAREA_TEST, 0);
            ...
            $files = $fs->get_area_files($this->assignment->get_context()->id,
                self::COMPONENT_NAME,
                ASSIGNSUBMISSION_mowaac_FILEAREA_TEST,
                0,
                'id',
                false);
            ...
            echo "about to post test file to web service";
            $file = reset($files);
            $url = $wsbaseaddress . "/test";
            $url = $wsbaseaddress;
            $this->mowaac_post_file($file, $url, "testFile");
    ...
    public function data_preprocessing(&$defaultvalues) {
        $draftitemid = file_get_submitted_draft_itemid('mowaactests');
        file_prepare_draft_area($draftitemid, $this->assignment->get_context()->id,
            self::COMPONENT_NAME, ASSIGNSUBMISSION_mowaac_FILEAREA_TEST,
            0, array('subdirs' => 0));
        $defaultvalues['mowaactests'] = $draftitemid;
    ...
    public function get_form_elements_for_user($submissionorgrade, MoodleQuickForm $mform, stdClass $data, $userid) {
        ...
        $data = file_prepare_standard_filemanager($data,
            ...
            self::COMPONENT_NAME,
            ASSIGNSUBMISSION_mowaac_FILEAREA_SUBMISSION,
            $submissionid);

        $name = get_string("mowaac_submission", self::COMPONENT_NAME);
    ...
    public function save(stdClass $submission, stdClass $data) {
        echo"saving and posting submission to web service";
        global $DB;
        ...
        $mowaacsubmission = $this->get_mowaac_submission($submission->id);

        if ($mowaacsubmission) {
            // If there are old results, delete them.
            $this->delete_test_data($mowaacsubmission->id);
        } else {
            $mowaacsubmission = new stdClass();
            $mowaacsubmission->submission_id = $submission->id;
            $mowaacsubmission->assignment_id = $this->assignment->get_instance()->id;
            $mowaacsubmission->id = $DB->insert_record(self::TABLE_ASSIGNSUBMISSION_mowaac, $mowaacsubmission);
        }
        ...
        $response = $this->mowaac_post_file($file, $url, "taskFile");

        if (empty($response)) {
            echo "no response from web service for sending task file for testing";
            return true;
        }
        ...
        foreach ($testresults as $tr) {
            ...
            $testresult->mowaac_id = $mowaacsubmission->id;

            $testresult->id = $DB->insert_record(self::TABLE_mowaac_TESTRESULT, $testresult);
            ...
            foreach ($testfailures as $tf) {
                ...
                $testfailure->id = $DB->insert_record(self::TABLE_mowaac_TESTFAILURE, $testfailure);
            }
        }
        ...
        foreach ($compilationerrors as $ce) {
            ...
            $compilationerror->mowaac_id = $mowaacsubmission->id;

            $compilationerror->id = $DB->insert_record(self::TABLE_mowaac_COMPILATIONERROR, $compilationerror);
        }
     ...
     * @return mixed
     */
    private function mowaac_post_file($file, $url, $paramname) {
        ...
        if ($info["http_code"] == 200) {
            echo "successfully posted to web service";
            return $response;
        }

        // Something went wrong.
        echo "Post file to mowaac web service was not successful";
        debugging("mowaac: Post file to server was not successful: http_code=" . $info["http_code"]);
    ...
    private function view_grading_summary(stdClass $submission, & $showviewlink) {
        ...
        $mowaacsubmission = $DB->get_record(self::TABLE_ASSIGNSUBMISSION_mowaac, array("submission_id" => $submission->id));
        $testresults = $DB->get_records(self::TABLE_mowaac_TESTRESULT, array("mowaac_id" => $mowaacsubmission->id));
        $comperrorcount = $DB->count_records(self::TABLE_mowaac_COMPILATIONERROR, array("mowaac_id" => $mowaacsubmission->id));
        $result = $this->get_short_testresult_overview($testresults, $comperrorcount);
        $result = html_writer::div($result, "submissionmowaacgrading");
    ...
    public function view(stdClass $submission) {
        ...
        $mowaacsubmission = $DB->get_record(self::TABLE_ASSIGNSUBMISSION_mowaac, array("submission_id" => $submission->id));
        $testresults = $DB->get_records(self::TABLE_mowaac_TESTRESULT, array("mowaac_id" => $mowaacsubmission->id));
        $compilationerrors = $DB->get_records(self::TABLE_mowaac_COMPILATIONERROR, array("mowaac_id" => $mowaacsubmission->id));
        ...
        foreach ($testresults as $tr) {
            ...
            $testfailures = $DB->get_records(self::TABLE_mowaac_TESTFAILURE, array("testresult_id" => $tr->id));
            if ($testfailures) {
        ...
        $html = html_writer::div($html, "mowaac_submission_view");
        return $html;
    }
    ...
    public function delete_instance() {
        ...
        $mowaac = $DB->get_record(self::TABLE_ASSIGNSUBMISSION_mowaac, array('assignment_id' => $assignmentid), "id");

        if ($mowaac) {
            $this->delete_test_data($mowaac->id);
        }

        // Delete mowaac assignment.
        $DB->delete_records(self::TABLE_ASSIGNSUBMISSION_mowaac, array("assignment_id" => $assignmentid));
    ...
    private function delete_test_data($mowaacid) {
        global $DB;

        $testresult = $DB->get_record(self::TABLE_mowaac_TESTRESULT, array("mowaac_id" => $mowaacid), "id", IGNORE_MISSING);
        if (!$testresult) {
            return true;
        }

        // Delete compilation errors.
        $DB->delete_records(self::TABLE_mowaac_COMPILATIONERROR, array("mowaac_id" => $mowaacid));

        // Delete test failures.
        $DB->delete_records(self::TABLE_mowaac_TESTFAILURE, array("testresult_id" => $testresult->id));

        // Delete test results.
        $DB->delete_records(self::TABLE_mowaac_TESTRESULT, array("mowaac_id" => $mowaacid));
    ...
    public function is_empty(stdClass $submission) {
        return $this->count_files($submission->id, ASSIGNSUBMISSION_mowaac_FILEAREA_SUBMISSION) == 0;
    }
